...
const ChatSlice = createSlice({
  ...
  reducers: {
    newChat: (
      state,
      action: PayloadAction<{ chat: ChatListItemInterface }>,
    ) => {
      const { chat } = action.payload;

      const idx = state.chats.findIndex((c) => c._id === chat._id);
      if (idx !== -1) {
        // Already present (e.g. added by the createUserChat mutation) — just refresh it
        state.chats[idx] = chat;
      } else {
        state.chats = [chat, ...state.chats];
      }

      indexDBStorage.set(DBStorageKeys.Chats, {
        id: "all_chats",
        data: snap(state.chats),
      });
    },
    ...
    onMessageReceived: (
      state,
      action: PayloadAction<{ data: ChatMessageInterface }>,
    ) => {
      ...
      const isDuplicate = state.chatMessages[chatId].some(
      ...
      const existingIndex = state.chatMessages[chatId].findIndex((msg) => {
        const isTemp =
          msg._id.toString().startsWith("temp-") || msg.status === "queued";
        return (
          isTemp &&
    ...
    updateMessageReactions: (
      ...
      const msgIdx = state.chatMessages[chatId].findIndex(
      ...
      if (
        state.currentChat?.lastMessage?._id === messageId &&
    ...
    updateChatLastMessage: (
      state,
      action: PayloadAction<ChatMessageUpdateInterface>,
    ) => {
    ...
    updateMessageDelivery: (
      ...
      if (
        chatIdx !== -1 &&
        state.chats[chatIdx].lastMessage?._id === messageId
      ) {
    ...
    markMessagesAsSeen: (
      ...
      state.chatMessages[chatId] = state.chatMessages[chatId].map((msg) => {
        if (!messageIds.includes(msg._id) || msg.sender._id === seenBy)
          return msg;
        const seenByArr = msg.seenBy || [];
        return {
          ...msg,
          seenBy: seenByArr.includes(seenBy)
            ? seenByArr
            : [...seenByArr, seenBy],
          status: "seen",
        };
  ...
  extraReducers: (builder) => {
    builder.addMatcher(
      ChatApiSlice.endpoints.getUserChats.matchFulfilled,
      (state, action) => {
    ...
    builder.addMatcher(
      ChatApiSlice.endpoints.getChatMessages.matchFulfilled,
      (state, action) => {
        const { data } = action.payload;
    ...
    builder.addMatcher(
      ChatApiSlice.endpoints.getAvailableUsers.matchFulfilled,
      (state, action) => {
        const { data } = action.payload;
        state.users = data;
        indexDBStorage.set(DBStorageKeys.Users, {
          id: "all_users",
          data: snap(data),
        });
      },
    ...
    builder.addMatcher(
      ChatApiSlice.endpoints.createUserChat.matchFulfilled,
      (state, action) => {
        const { data } = action.payload;
    ...
    builder.addMatcher(
      ChatApiSlice.endpoints.createGroupChat.matchFulfilled,
      (state, action) => {
        const { data } = action.payload;
        const idx = state.chats.findIndex((c) => c._id === data._id);
        if (idx !== -1) {
