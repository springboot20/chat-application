            ...
            <div className="relative flex flex-col md:flex-row bg-white dark:bg-gray-900 w-full min-h-screen md:min-h-[600px] md:max-h-[85vh]">
              ...
              <div className="flex-1 overflow-y-auto bg-white dark:bg-gray-900">
                <AnimatePresence mode="wait">
                  ...
                  {activeTab === "new-group" && (
                    <NewGroupList key="group" onClose={handleClose} />
                  )}
                  {activeTab === "discover" && (
                    <DiscoverUsersList key="discover" />
                  )}
                </AnimatePresence>
              </div>
            </div>
...
const ExistingContactList = ({ onClose }: { onClose: () => void }) => {
  ...
  const [toggleBlock, { isLoading: isBlocking }] =
    useToggleBlockContactMutation();
  const [createUserChat, { isLoading: isCreatingChat }] =
    useCreateUserChatMutation();
...
const NewGroupList = ({ onClose }: { onClose: () => void }) => {
  ...
  const [createNewGroupChat, { isLoading: isCreating }] =
    useCreateGroupChatMutation();
  const [toggleBlock, { isLoading: isBlocking }] =
    useToggleBlockContactMutation();
...
const DiscoverUsersList = () => {
  ...
  const [selectedCategory, setSelectedCategory] =
    useState<ContactCategoryType>("friend");
