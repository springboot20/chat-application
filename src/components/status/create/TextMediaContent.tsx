...
const TextEditorContent = () => {
  ...
  const canPostTextStatus =
    mediaContentType === "text" && textContent.length > 0;
  ...
  const isPosting = isAddingNewTextStatus;
        ...
        <div className="flex items-center justify-between gap-4 bg-gray-900/60 backdrop-blur-md p-2 rounded-full border border-white/10">
          ...
          <div className="flex items-center shrink-0 gap-x-3">
            <motion.button
              ...
              title="Send message"
              type="button"
              disabled={isPosting || !canPostTextStatus}
...
const ColorModal = ({ open, close }: { open: boolean; close: () => void }) => {
  const { colors, handleSelectedTextBackground, selectedTextBackground } =
    useStatusStories();
                    ...
                    return (
                      <button
                        ...
                        className={classNames(
                          ...
                          isActive &&
                            "ring-2 ring-offset-2 ring-offset-slate-600 ring-indigo-500",
                        )}
