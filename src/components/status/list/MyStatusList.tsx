...
export const MyStatusListComponent: React.FC<StatusListProps> = ({
  ...
  const { data: myStatusData, isLoading: isMyStatusLoading } =
    useGetUserStatusesQuery(undefined, {
          ...
          {hasStatus ? (
            <div className="overflow-x-hidden">
              ...
              <motion.div
                role="button"
                onClick={handleClick}
                className="rounded-xl overflow-hidden border dark:border-gray-600/30 w-full p-2 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors"
              >
                <div className="w-full h-full flex items-center gap-x-3">
                  ...
                  <div className="relative">
                    <div className="rounded-full relative size-16 border-2 border-gray-300 dark:border-gray-600">
                      <UserAvatar
                        imageUrl={
                          myStatus.user?.avatar?.url || user?.avatar?.url || ""
                        }
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex-shrink-0">
                    <StatusPreview
                      status={myStatus.items[myStatus?.items?.length - 1]}
                    />
                  </div>
                </div>
              </motion.div>
...
export const MyStatusRow = ({ myStatus, onViewStatus }: MyStatusRowProps) => {
  ...
  return hasStatus ? (
    <motion.div
      role="button"
      onClick={handleClick}
      className="rounded-xl overflow-hidden border dark:border-gray-600/30 w-full h-full p-2 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors"
    >
      <div className="w-full h-full flex items-center gap-x-3">
          ...
          {hasStatus ? (
            <div className="flex items-center gap-3 mt-0.5">
              ...
              <div className="flex items-center gap-1 text-xs text-gray-500 dark:text-gray-500">
                <EyeIcon height={14} width={14} />
                <span>
                  {myStatus?.items?.reduce(
                    (acc, item) => acc + (item.viewCount || 0),
                    0,
                  )}{" "}
                  views
                </span>
        ...
        {!hasStatus && (
          <CameraIcon className="text-gray-400" height={20} width={20} />
        )}
      </div>
    </motion.div>
  ) : (
...
export const MyStatusListModal: React.FC<MyStatusListModalProps> = ({
  myStatus,
  onClose,
}) => {
        ...
        <div className="my-2 space-y-3">
          {myStatus?.items?.map((status, index) => (
            <motion.div
              ...
              className="rounded-xl overflow-hidden border dark:border-gray-600/30 w-full h-[72px] p-2 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors flex items-center gap-x-2"
            >
              {/* Preview */}
