        ...
        {isOwnStatus && (
          <>
            <button
              ...
              className="flex items-center gap-1.5 text-white/80 hover:text-white transition-colors px-2.5 py-2 rounded-lg hover:bg-white/10"
            >
...
export const ViewStatusWindowPanelSlot = () => {
  ...
  const longPressTimeoutRef = useRef<ReturnType<typeof setInterval> | null>(
    null,
  );
  ...
  }, [handlePrevious, handleNext, handleStatusWindowChange, showViewers]);
    ...
    if (window.confirm("Delete this status?")) {
      try {
        ...
        if (statuses.length === 1) {
          ...
          const updatedItems: typeof statuses = statuses?.filter(
            (status: StatusGroup) => status._id !== currentStatus?._id,
          );
          const nextIndex: number =
            activeStatusIndex >= updatedItems?.length
        ...
        <div className="w-full max-w-3xl h-full max-h-[90vh] flex flex-col">
          ...
          <div className="px-4">
            <StatusViewerHeader
              ...
              viewCount={
                selectedStatusToView?.items?.[activeStatusIndex]?.viewCount || 0
              }
              onClose={() => {
          ...
          <div className="flex-1 relative overflow-hidden rounded-2xl shadow-2xl bg-gray-950">
            ...
            {currentStatus.type === "image" &&
              currentStatus.mediaContent?.url && (
                <>
            ...
            {currentStatus.type === "text" &&
              currentStatus.textContent?.backgroundColor && (
                <div
