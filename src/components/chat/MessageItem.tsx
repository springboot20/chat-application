...
export const MessageItem: React.FC<MessageItemProps> = ({
  ...
  const [currentMessageImageIndex, setCurrentMessageImageIndex] =
    useState<number>(-1);
  ...
  const indicatorVisible = useTransform(x, (val) =>
    isDragging || Math.abs(val) > 0 ? 1 : 0,
  );
  ...
  const handleSelectReactionEmoji = useCallback(
    async (key: string, emojiData: EmojiClickData, event: MouseEvent) => {
      ...
      const userPreviouslyUsedThisEmoji = currentReactions.some(
        (r) =>
          r.emoji === emojiData.emoji && r.userIds?.includes(String(user?._id)),
      );
      ...
      if (!userPreviouslyUsedThisEmoji) {
        ...
        if (existingEmojiIndex !== -1) {
          optimisticReactions[existingEmojiIndex].userIds.push(
            String(user?._id),
          );
  ...
  useEffect(() => {
    if (messageToReply !== message._id || message.isDeleted) return;
  ...
  }, [messageToReply, message._id, message.isDeleted]);
  ...
  const getReactionStats = useCallback((): ReactionStats => {
    ...
    const userHasReacted: boolean = categorizedReactionMemoized.some(
      (r: CategorizedReaction) => hasUserReacted(r, user?._id || ""),
    );
    ...
    return (
      ...
      >
        {categorizedReactions
          .slice(0, 3)
          .map((reaction: CategorizedReaction, index: number) => (
            <span
              ...
              return (
                <div
                  ...
                  className={classNames(
                    ...
                    count === 1
                      ? "aspect-auto max-h-[350px] min-w-[200px]"
                      : "aspect-square",
          ...
          <div className="flex flex-col gap-1 p-1">
            {otherAttachments.map((file, index) => (
              <DocumentPreview
                ...
                showOverlay={true}
                isOwnedMessage={Boolean(isOwnedMessage)}
                uploadProgress={fileProgress?.get(
                  mediaAttachments.length + index,
                )}
                status={message.status}
              />
            ))}
        ...
        >
            ...
            >
              <div className="relative shadow-2xl rounded-lg overflow-hidden">
                <EmojiPicker
                  ...
                  height={
                    window.innerWidth < 768
                      ? Math.min(400, window.innerHeight - 20)
                      : 400
                  }
                  lazyLoadEmojis
                />
          ...
          {currentChat.participants.length > 2 &&
            ...
            (message.sender?.avatar?.url ? (
              ...
              <div
                style={{
                  backgroundColor: getDynamicUserColor(
                    message?.sender?._id,
                    theme === "dark",
                  ),
                }}
