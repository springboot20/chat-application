...
export const MessageTabComponent: React.FC<MessageTabComponentProps> = ({
  close,
}) => {
  ...
  const { chatsWithMeta, isLoadingChats, refetchChats, currentChat } =
    useChat();
  ...
  const chatCounts = useMemo(() => {
    ...
    const unread =
      chatsWithMeta?.filter((chat: any) => chat?.unreadCount > 0).length || 0;
  ...
  return (
    <>
      ...
      <div
        id="chat-navigation"
        className="fixed lg:left-20 w-[25rem] bg-white dark:bg-black flex-1 border-r border-gray-200 dark:border-white/10 h-screen z-50 hidden lg:block"
      >
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div id="mobile-chat-list" className="h-full w-full">
            <div className="px-4 pt-4">
              {/* Search */}
              <div
                id="mobile-search"
