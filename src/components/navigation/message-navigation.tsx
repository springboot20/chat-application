...
export const MessageNavigation: React.FC<{
  ...
  ({
    ...
    //   switch (filter) {
    //     case "unread":
    //       return filteredfilter((n) => !n.isRead);
    //     case "read":
    //       return filteredfilter((n) => n.isRead);
    //     default:
    //       return filtered
    ...
    const filteredChats = useMemo(() => {
      ...
      switch (filter) {
        case "unread":
          filtered = filtered?.filter((n) => getUnreadCount(n._id) > 0);
          break;
      ...
      return filtered;
    }, [chats, filter, getUnreadCount, localSearchQuery, user]);

    const chatCounts = useMemo(() => {
      const all = chats?.length || 0;
      const unread = chats?.filter((chat) => getUnreadCount(chat._id) > 0).length || 0;
