...
const MessageInput = ({
  ...
  const shouldShowMicButton =
    ...
    !hasFiles &&
    !hasReviewableAudio &&
    (!isActivelyRecording || isDraggingMic); // Keep visible while dragging
  ...
  const handleSendVoiceMessage = async () => {
    ...
    let durationToSend = audioDuration;

    if (isRecording) {
      blobToSend = await stopRecording(); // waits for the real onstop event
      durationToSend = durationRef.current; // read from the ref, not stale state
    }

    if (!blobToSend || !currentChat?._id) {
      resetRecording(); // ✅ don't leave a ghost recorder bar
      resetLock();
      return;
    }
    ...
    const voiceFile = new File([blobToSend], `voice-${Date.now()}.webm`, {
    ...
    dispatch(
      onMessageReceived({
        data: optimisticMessage as unknown as ChatMessageInterface,
      }),
    );
            ...
            <div
              ...
              style={{
                borderLeftColor: getDynamicUserColor(
                  reduxStateMessages.find((msg) => msg._id === messageToReply)
                    ?.sender?._id || "",
                  theme === "dark",
                ),
              }}
            >
              <div className="flex-1 p-2 min-w-0">
                {(() => {
                  ...
                  const attachmentFile = replyMessage
              ...
              {(() => {
                ...
                const firstAttachment = replyMessage
                  ?.attachments?.[0] as Attachment;
